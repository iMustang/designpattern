...
public class LazyDoubleCheckSingleton {
     ...
     *
     */
    private volatile static LazyDoubleCheckSingleton instance; // volatile防止指令重排序，volatile阻止了被修饰变量的访问前后指令重排，保证了指令执行顺序。
    ...
    public static LazyDoubleCheckSingleton getInstance() {
        if (instance == null) { // 刚开始，线程A、B都判断为空，假设线程A先进入临界区
            synchronized (LazyDoubleCheckSingleton.class) {
                ...
                // 进行第二次判空是为了，线程B在外层if判断为true也会进入临界区，此时instance已经由线程A构建，不再为空，instance是静态变量，线程B再访问一次，判断不为空，不再构建instance
                if (instance == null) {
